...
export function AppShell() {
  ...
  // Center file workspace + right chat panel
  const [fileTabs, setFileTabs] = useState<Tab[]>([]);
  const [activeFileTabId, setActiveFileTabId] = useState<string | null>(null);
  const [rightPanelOpen, setRightPanelOpen] = useState(true);

  const voiceChat = useVoiceChat({
    onTranscript: useCallback((text: string, sendImmediately?: boolean) => {
      if (sendImmediately) {
        chatInputRef.current?.insertAndSend(text);
        return;
      }
      chatInputRef.current?.insertText(text);
    }, []),
    canStartPushToTalk: useCallback(() => {
      const active = document.activeElement;
      const tagName = active?.tagName?.toLowerCase();
      const inAllowedField =
        !active ||
        active === document.body ||
        tagName === "textarea";
      return inAllowedField && (chatInputRef.current?.isEmpty() ?? true);
    }, []),
  });
  ...
  return (
    ...
    <div style={{ display: "flex", height: "100dvh", overflow: "hidden", background: "var(--bg)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0, order: 1 }}>
      ...
      >
        {voiceChat.enabled && voiceChat.bubbleMode && (
          <div
            style={{
              position: "absolute",
              top: 52,
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, fontSize: 12, opacity: 0.9 }}>
              <span
                style={{
                  width: 8,
                  height: 8,
                  borderRadius: "50%",
                  background:
                    voiceChat.bubbleMode === "listening"
                      ? "#ef4444"
                      : voiceChat.bubbleMode === "confirming"
                      ? "#f59e0b"
                      : "#60a5fa",
                  boxShadow:
                    voiceChat.bubbleMode === "listening"
        ...
        <div ref={topBarRef} style={{ display: "flex", alignItems: "center", flexShrink: 0, borderBottom: "1px solid var(--border)", height: 36, background: "var(--bg-panel)" }}>
          ...
          {showChat && (
            <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
              ...
              <VoiceControls controller={voiceChat} />
            </div>
          )}
    ...
    {/* Chat panel toggle — always visible at top-right */}
    <button
