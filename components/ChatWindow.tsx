...
export function ChatWindow({ session, newSessionCwd, onAgentEnd, onSessionCreated, onSessionForked, modelsRefreshKey, chatInputRef, voiceChat, onBranchDataChange, onSystemPromptChange, onSessionStatsChange, onContextUsageChange }: Props) {
  const {
  ...
  const spokenAssistantKeyRef = useRef<string | null>(null);
  ...
  const latestAssistantMessage = [...messages].reverse().find((message): message is AgentMessage & { role: "assistant" } => message.role === "assistant");
  ...
  const latestAssistantKey = latestAssistantMessage
    ? `${latestAssistantMessage.timestamp ?? 0}:${latestAssistantText}`
    : null;

  useEffect(() => {
    spokenAssistantKeyRef.current = null;
  }, [session?.id]);

  useEffect(() => {
    if (!voiceChat?.enabled || !voiceChat.speechEnabled || !voiceChat.autoSpeakEnabled) return;
    if (!latestAssistantKey || !latestAssistantText) return;
    if (agentRunning || streamState.isStreaming) return;
    if (spokenAssistantKeyRef.current === latestAssistantKey) return;
    spokenAssistantKeyRef.current = latestAssistantKey;
    voiceChat.speakText(latestAssistantText);
  }, [
    agentRunning,
    latestAssistantKey,
    latestAssistantText,
