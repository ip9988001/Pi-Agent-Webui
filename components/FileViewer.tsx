...
function TextFileViewer({ filePath, cwd }: Props) {
  ...
  const dataContentRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    dataContentRef.current = data?.content ?? null;
  }, [data?.content]);
    ...
    return fetch(`/api/files/${encoded}?type=read`)
      ...
      .then((d: FileData & { error?: string }) => {
        ...
        if (isRefresh) {
          ...
          setDraftContent((prev) => (editModeRef.current && prev !== dataContentRef.current ? prev : d.content));
  ...
  const hasDiff = data ? prevContent !== null && prevContent !== data.content : false;
  const isDirty = data ? draftContent !== data.content : false;
  ...
  const handleSave = useCallback(async (mode: "manual" | "auto" = "manual") => {
    ...
    setSaving(true);
    setError(null);
    setSaveMessage(mode === "auto" ? "Auto-saving..." : null);
    try {
      ...
      setData((prev) => prev ? { ...prev, content: draftContent, size: result.size ?? prev.size } : prev);
      setPrevContent(data.content);
      setChangeCount((c) => c + 1);
      setSaveMessage(mode === "auto" ? "Auto-saved" : "Saved");
    } catch (e) {
      setError(String(e));
    } finally {
      setSaving(false);
    }
  }, [data, draftContent, filePath]);
  ...
  }, [editMode, handleSave, isDirty, saving]);

  useEffect(() => {
    if (!editMode || !data || !isDirty || saving) return;
    const timer = window.setTimeout(() => {
      void handleSave("auto");
    }, AUTOSAVE_DELAY_MS);
    return () => window.clearTimeout(timer);
  }, [AUTOSAVE_DELAY_MS, data, editMode, handleSave, isDirty, saving]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        {saveMessage && <span style={{ color: "#4ade80" }}>{saveMessage}</span>}
        ...
        {viewMode === "source" && (
          <>
            <button
              ...
              title={isDirty ? "Save changes (Ctrl+S / Cmd+S)" : "No changes to save"}
              style={{
                padding: "2px 8px",
                fontSize: 11,
                cursor: saving || !isDirty ? "not-allowed" : "pointer",
                background: "rgba(34,197,94,0.15)",
                color: isDirty ? "#4ade80" : "var(--text-dim)",
                border: "1px solid rgba(34,197,94,0.35)",
                borderRadius: 5,
                opacity: saving || !isDirty ? 0.65 : 1,
              }}
            >
              {saving ? "Saving..." : "Save"}
            </button>
            ...
            <span style={{ color: isDirty ? "#f59e0b" : "var(--text-dim)" }}>
              {isDirty ? "unsaved" : "editing"}
            </span>
          </>
        )}
